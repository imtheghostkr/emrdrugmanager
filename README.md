...
- `group_same=true` to combine rows by same ingredient and dose (enabled by default in the Web UI).
- `round_order_qty_to_100=true` to round each non-zero recommended order quantity up to the next 100 units (on by default).
- `exclude_outside=true` to exclude outside-prescription drugs (enabled by default in the Web UI).
- `exclude_injection=true` to exclude injection drugs (enabled by default in the Web UI).
